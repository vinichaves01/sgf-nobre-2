...
async function exigirAdministrador(request: NextRequest) {
  ...
  if (!token) {
    return { erro: "Sessão não informada.", status: 401 } as const;
  }
  ...
  return { adminClient, usuarioAtualId: userData.user.id } as const;
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: authData, error: authError } =
      await acesso.adminClient.auth.admin.listUsers({ page: 1, perPage: 1000 });
    ...
    const { data: perfis, error: perfisError } = await acesso.adminClient
      .from("perfis")
      .select("id, nome, tipo, ativo, motorista_id, updated_at");

    if (perfisError) {
      return NextResponse.json({ erro: perfisError.message }, { status: 400 });
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!nome || !email || senha.length < 6) {
      return NextResponse.json(
        { erro: "Informe nome, e-mail e uma senha com pelo menos 6 caracteres." },
        { status: 400 }
      );
    }
    ...
    if (tipo === "Motorista" && !motoristaId) {
      return NextResponse.json(
        { erro: "Vincule a conta a um motorista cadastrado." },
        { status: 400 }
      );
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (acao === "redefinir_senha") {
      ...
      const { error } = await acesso.adminClient.auth.admin.updateUserById(id, {
        password: senha,
      });
      ...
      const email = String(body.email ?? "").trim().toLowerCase();
      if (!email) {
        return NextResponse.json({ erro: "Informe o novo e-mail." }, { status: 400 });
      }
      const { error } = await acesso.adminClient.auth.admin.updateUserById(id, {
        email,
        email_confirm: true,
      ...
      if (id === acesso.usuarioAtualId && !ativo) {
        return NextResponse.json(
          { erro: "Você não pode bloquear a conta que está usando." },
          { status: 400 }
        );
      }
      if (tipo === "Motorista" && !motoristaId) {
